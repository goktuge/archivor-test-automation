...
export class InfluxDBReporter implements Reporter {
  ...
  private isConfigured(): boolean {
    const url = process.env.INFLUX_URL || process.env.INFLUXDB_URL;
  ...
  onBegin(_config: FullConfig, _suite: Suite): void {
    ...
    try {
      ...
      const org = process.env.INFLUX_ORG || process.env.INFLUXDB_ORG || 'archivor';
      const bucket = process.env.INFLUX_BUCKET || process.env.INFLUXDB_BUCKET || 'playwright_metrics';
